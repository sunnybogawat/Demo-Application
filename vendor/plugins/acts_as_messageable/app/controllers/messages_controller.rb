class MessagesController < ApplicationController
  ...
  def post
    # Reinitialize recipients list if validation triggers.
    @users = User.possible_recipients_for_message(@message)
    ...
    if !@message.recipients.empty? and @message.post 
      respond_to do |format|
  ...
  def show
    @conversation = @message_box.conversations.find(params[:id]) rescue nil
    @user = @conversation.user.messageable #Need to change modeling name.
